...
export class BenedictComponent implements OnInit {
    ...
    calculateMifflin() {
        if (this.age && this.weight && this.height) {
            this.answerFemale = this.dataService.harris(this.weight, this.height, this.age, 0);
            this.dropTapEllieFly();
        }
        if (this.weight && this.height) {
            this.answerMale = this.dataService.harris(this.weight, this.height, this.age, 1);
        }
    }
    dropTapEllieFly() {
